class ImageHover {
  ...
  updateParallax() {
    const deltaX = (this.mouseX - this.centerX) / (this.DOM.el.offsetWidth / 2);
    const deltaY =
  ...
  updateDebugInfo() {
    ...
    if (rotationInfo) {
      rotationInfo.textContent = `rotation: ${this.currentRotation.toFixed(1)}`;
    }
    if (scaleInfo) {
  ...
  createHoverTimeline() {
    ...
    this.hoverTimeline = gsap.timeline({ paused: true }).to(reversedElements, {
      scale: (i, target) => {
        const originalIndex = Array.from(this.DOM.innerElems).indexOf(target);
